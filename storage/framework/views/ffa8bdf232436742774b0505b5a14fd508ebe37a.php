...
<body class="bg-slate-50 text-slate-800 min-h-screen flex flex-col md:flex-row">
    ...
    <aside class="hidden md:flex flex-col w-72 bg-slate-900 text-slate-100 min-h-screen fixed top-0 left-0 z-30 shadow-xl border-r border-slate-800">
        ...
        <div class="p-5 border-b border-slate-800 bg-slate-955/10">
            <div class="flex items-center space-x-3 mb-4">
                <span class="h-10 w-10 rounded-full bg-indigo-500/10 border border-indigo-500/20 text-indigo-400 flex items-center justify-center font-extrabold text-base flex-shrink-0">
                    <?php echo e(substr($customer->user->username ?? 'C', 0, 1)); ?>

                </span>
                <div class="overflow-hidden">
                    <h4 class="text-xs font-bold text-slate-200 truncate"><?php echo e($customer->user->username ?? 'N/A'); ?></h4>
                    <p class="text-[10px] text-slate-500 truncate font-mono"><?php echo e($customer->user->email ?? 'N/A'); ?></p>
                </div>
            </div>
            ...
            <div class="bg-slate-800/50 rounded-xl p-3 space-y-2 text-[11px]">
                <div class="flex justify-between items-center">
                    ...
                    <span class="font-extrabold text-indigo-400"><?php echo e($customer->membership_level ?? 'Standard'); ?></span>
                </div>
                <div class="flex justify-between items-center">
                    <span class="text-slate-400">Điểm tích lũy:</span>
                    <span class="font-bold text-amber-400"><?php echo e(number_format($customer->points ?? 0)); ?> Pts</span>
                </div>
            </div>
        </div>
        ...
        <nav class="flex-grow p-4 space-y-1">
            <a href="<?php echo e(route('customer.dashboard')); ?>" class="flex items-center px-4 py-3 text-xs font-bold rounded-xl transition-all duration-150 <?php echo e(request()->routeIs('customer.dashboard') ? 'bg-blue-600 text-white shadow-md shadow-blue-600/10' : 'text-slate-400 hover:bg-slate-800 hover:text-white'); ?>">
                <svg class="mr-3 h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" />
                </svg>
                Trang tổng quan
            </a>
            <a href="<?php echo e(route('customer.orders.create')); ?>" class="flex items-center px-4 py-3 text-xs font-bold rounded-xl transition-all duration-150 <?php echo e(request()->routeIs('customer.orders.create') ? 'bg-blue-600 text-white shadow-md shadow-blue-600/10' : 'text-slate-400 hover:bg-slate-800 hover:text-white'); ?>">
                <svg class="mr-3 h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v3m0 0v3m0-3h3m-3 0H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
                Tạo đơn hàng mới
            </a>
            <a href="<?php echo e(route('customer.vouchers')); ?>" class="flex items-center px-4 py-3 text-xs font-bold rounded-xl transition-all duration-150 <?php echo e(request()->routeIs('customer.vouchers') ? 'bg-blue-600 text-white shadow-md shadow-blue-600/10' : 'text-slate-400 hover:bg-slate-800 hover:text-white'); ?>">
                <svg class="mr-3 h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 5v2m0 4v2m0 4v2M5 5a2 2 0 00-2 2v3a2 2 0 110 4v3a2 2 0 002 2h14a2 2 0 002-2v-3a2 2 0 110-4V7a2 2 0 00-2-2H5z" />
                </svg>
                Mã giảm giá
            </a>
            <a href="<?php echo e(route('customer.profile')); ?>" class="flex items-center px-4 py-3 text-xs font-bold rounded-xl transition-all duration-150 <?php echo e(request()->routeIs('customer.profile') ? 'bg-blue-600 text-white shadow-md shadow-blue-600/10' : 'text-slate-400 hover:bg-slate-800 hover:text-white'); ?>">
                <svg class="mr-3 h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="flex-1 flex flex-col md:pl-72 min-h-screen pb-20 md:pb-0">
        ...
        <header class="h-16 bg-white border-b border-slate-200/80 sticky top-0 z-20 px-6 flex items-center justify-between shadow-sm">
            <div class="flex items-center space-x-3">
                ...
                <h2 class="text-sm md:text-base font-bold text-slate-800"><?php echo $__env->yieldContent('header_title', 'Trang Khách Hàng'); ?></h2>
            </div>
            ...
            <div class="flex items-center space-x-4">
                ...
                <div class="relative group">
                    ...
                    <div class="absolute right-0 mt-2 w-72 bg-white rounded-xl shadow-lg border border-slate-100 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50">
                        <div class="p-3 border-b border-slate-50 flex justify-between items-center">
                            ...
                            <?php if(Auth::check() && Auth::user()->unreadNotifications->count() > 0): ?>
                                <a href="<?php echo e(route('notifications.markAllRead')); ?>" class="text-[10px] text-blue-600 hover:underline">Đánh dấu đã đọc</a>
                            <?php endif; ?>
                        </div>
                            ...
                            <?php if(Auth::check() && Auth::user()->unreadNotifications->count() > 0): ?>
                                <?php $__currentLoopData = Auth::user()->unreadNotifications->take(5); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $notification): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <div class="p-3 border-b border-slate-50 hover:bg-slate-50/50 transition">
                                        <div class="text-[10px] font-bold text-slate-800"><?php echo e($notification->data['title'] ?? 'Thông báo'); ?></div>
                                        <div class="text-[10px] text-slate-500 mt-0.5"><?php echo e($notification->data['message'] ?? ''); ?></div>
                                        <div class="text-[9px] text-slate-400 mt-1"><?php echo e($notification->created_at->diffForHumans()); ?></div>
                                    </div>
                ...
                <div class="hidden sm:flex items-center bg-amber-50 border border-amber-100 rounded-full px-3 py-1 text-[11px] font-bold text-amber-700">
                    ...
                    <span><?php echo e(number_format($customer->points ?? 0)); ?> Pts</span>
                </div>
                ...
                <div class="flex items-center space-x-2.5">
                    <div class="text-right hidden sm:block">
                        <span class="text-xs font-bold text-slate-800 block"><?php echo e($customer->user->username ?? 'N/A'); ?></span>
                        <span class="text-[9px] font-semibold text-slate-400 block tracking-wider uppercase"><?php echo e($customer->membership_level ?? 'Standard'); ?> Member</span>
                    </div>
    ...
    <nav class="md:hidden fixed bottom-0 left-0 right-0 h-16 bg-white/95 backdrop-blur-md border-t border-slate-200 flex items-center justify-around z-30 shadow-[0_-2px_10px_rgba(0,0,0,0.05)]">
        <a href="<?php echo e(route('customer.dashboard')); ?>" class="flex flex-col items-center justify-center flex-1 py-2 <?php echo e(request()->routeIs('customer.dashboard') ? 'text-blue-600 font-bold' : 'text-slate-400'); ?>">
            <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" />
            </svg>
            <span class="text-[9px] mt-1">Tổng quan</span>
        </a>
        <a href="<?php echo e(route('customer.orders.create')); ?>" class="flex flex-col items-center justify-center flex-1 py-2 <?php echo e(request()->routeIs('customer.orders.create') ? 'text-blue-600 font-bold' : 'text-slate-400'); ?>">
            <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v3m0 0v3m0-3h3m-3 0H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            <span class="text-[9px] mt-1">Đặt đơn</span>
        </a>
        <a href="<?php echo e(route('customer.vouchers')); ?>" class="flex flex-col items-center justify-center flex-1 py-2 <?php echo e(request()->routeIs('customer.vouchers') ? 'text-blue-600 font-bold' : 'text-slate-400'); ?>">
            <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 5v2m0 4v2m0 4v2M5 5a2 2 0 00-2 2v3a2 2 0 110 4v3a2 2 0 002 2h14a2 2 0 002-2v-3a2 2 0 110-4V7a2 2 0 00-2-2H5z" />
            </svg>
            <span class="text-[9px] mt-1">Khuyến mãi</span>
        </a>
        <a href="<?php echo e(route('customer.profile')); ?>" class="flex flex-col items-center justify-center flex-1 py-2 <?php echo e(request()->routeIs('customer.profile') ? 'text-blue-600 font-bold' : 'text-slate-400'); ?>">
            <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
            </svg>
...
<?php /**PATH C:\Users\danht\OneDrive\Desktop\toi_uu_giao_van\resources\views/layouts/customer.blade.php ENDPATH**/ ?>