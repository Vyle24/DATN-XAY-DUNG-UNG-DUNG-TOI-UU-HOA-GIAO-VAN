<?php $__env->startSection('header_title', 'Trang Tổng Quan'); ?>
...
<div class="bg-gradient-to-r from-blue-600 to-indigo-600 rounded-3xl p-6 md:p-8 text-white shadow-xl shadow-blue-500/10 mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
    <div>
        <h3 class="text-lg md:text-xl font-bold">Xin chào, <?php echo e($customer->user->username ?? 'Khách hàng'); ?>! 👋</h3>
    ...
    <div>
        <a href="<?php echo e(route('customer.orders.create')); ?>" class="inline-flex items-center justify-center px-5 py-2.5 bg-white text-blue-600 hover:bg-blue-50 transition font-bold text-xs rounded-xl shadow-sm">
...
<div class="grid grid-cols-2 lg:grid-cols-6 gap-4 mb-8">
    ...
    <div class="bg-white rounded-2xl p-4 border border-slate-200/60 shadow-sm">
        ...
        <span class="text-xl font-extrabold text-amber-600 mt-1 block"><?php echo e($stats['pending']); ?></span>
    ...
    <div class="bg-white rounded-2xl p-4 border border-slate-200/60 shadow-sm">
        ...
        <span class="text-[10px] text-slate-400 mt-1 block">Tỉ lệ: <?php echo e($stats['success_rate']); ?>%</span>
    </div>

    <div class="bg-white rounded-2xl p-4 border border-slate-200/60 shadow-sm">
        <span class="text-[10px] text-slate-400 font-bold uppercase tracking-wider block">Đã chi tiêu</span>
        <span class="text-xl font-extrabold text-blue-600 mt-1 block"><?php echo e(number_format($stats['total_spent'])); ?>đ</span>
    ...
    <div class="bg-white rounded-2xl p-4 border border-slate-200/60 shadow-sm">
        ...
        <span class="text-xl font-extrabold text-indigo-600 mt-1 block"><?php echo e(number_format($customer->points ?? 0)); ?></span>
        <span class="text-[10px] text-indigo-500 mt-1 block font-semibold">Hạng <?php echo e($customer->membership_level ?? 'Standard'); ?></span>
    </div>
</div>
    ...
    <div class="px-6 py-4 border-b border-slate-100 flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div class="flex items-center gap-2">
            <form id="filterForm" class="flex items-center gap-2">
                <input type="text" name="search" id="searchInput" placeholder="Tìm theo mã đơn..." value="<?php echo e(request('search')); ?>" class="px-3 py-1.5 text-xs bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 w-48 transition">
                <select name="status" id="statusSelect" class="px-3 py-1.5 text-xs bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 transition">
                    ...
                    <option value="processing" <?php echo e(request('status') == 'processing' ? 'selected' : ''); ?>>Đang giao</option>
                    <option value="delivered" <?php echo e(request('status') == 'delivered' ? 'selected' : ''); ?>>Thành công</option>
            ...
            <a href="<?php echo e(route('customer.orders.create')); ?>" class="inline-flex items-center px-3 py-1.5 bg-blue-600 text-white text-xs font-bold rounded-xl hover:bg-blue-700 transition whitespace-nowrap">
                + Đặt đơn mới
            </a>
        </div>
    </div>
    ...
    <div class="overflow-x-auto">
        <table class="w-full text-left border-collapse">
            ...
            <tbody id="orderList" class="divide-y divide-slate-100 text-xs text-slate-650">
                <?php echo $__env->make('customer.partials.order_list', ['recentOrders' => $recentOrders], \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
            </tbody>
        </table>
    </div>

    <?php if($recentOrders->hasMorePages()): ?>
...
<?php echo $__env->make('layouts.customer', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\Users\danht\OneDrive\Desktop\toi_uu_giao_van\resources\views/customer/dashboard.blade.php ENDPATH**/ ?>