<?php $__env->startSection('header_title', 'Quản Lý Điểm Điều Phối (Hubs)'); ?>
    ...
    <div class="lg:col-span-2 space-y-6">
        ...
        <div class="grid grid-cols-2 gap-4">
            ...
            <div class="bg-white rounded-2xl p-4 border border-slate-200/60 shadow-sm">
                ...
                <span class="text-xl font-extrabold text-blue-600 mt-1 block"><?php echo e($stats['total_orders']); ?></span>
        ...
        <!-- Search Form -->
        <form action="<?php echo e(route('admin.hubs')); ?>" method="GET" class="bg-white rounded-2xl border border-slate-200/60 shadow-sm p-4 flex items-center gap-4">
            <div class="relative w-full">
                <input type="text" name="search" value="<?php echo e(request('search')); ?>" placeholder="Tìm theo tên hoặc địa chỉ trạm..." class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-xs focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500">
                <?php if(request('search')): ?>
                    <a href="<?php echo e(route('admin.hubs')); ?>" class="absolute right-3 top-3 text-slate-400 hover:text-slate-650">✕</a>
                <?php endif; ?>
            </div>
        ...
        <div class="bg-white rounded-2xl border border-slate-200/60 shadow-sm overflow-hidden">
            <div class="overflow-x-auto">
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="divide-y divide-slate-100 text-sm text-slate-700">
                        <?php $__empty_1 = true; $__currentLoopData = $hubs; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $hub): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr class="hover:bg-slate-50/20 transition cursor-pointer" onclick="focusHub(<?php echo e($hub->id); ?>, <?php echo e($hub->latitude); ?>, <?php echo e($hub->longitude); ?>)">
                            <td class="px-6 py-4 font-bold text-slate-500">#<?php echo e($hub->id); ?></td>
                            <td class="px-6 py-4 font-bold text-slate-900">
                                <span><?php echo e($hub->name); ?></span>
                                <span class="block text-[10px] text-slate-400 font-mono mt-0.5"><?php echo e($hub->latitude); ?>, <?php echo e($hub->longitude); ?></span>
                            </td>
                            <td class="px-6 py-4 text-slate-600 text-xs max-w-[200px] truncate" title="<?php echo e($hub->address); ?>"><?php echo e($hub->address); ?></td>
                            <td class="px-6 py-4 text-right text-xs font-bold space-x-2" onclick="event.stopPropagation()">
                                <button onclick="openEditModal(<?php echo e(json_encode($hub)); ?>)" class="text-blue-600 hover:text-blue-800 hover:underline">Sửa</button>
                                <form action="<?php echo e(route('admin.hubs.destroy', $hub->id)); ?>" method="POST" class="inline" onsubmit="return confirm('Bạn có chắc chắn muốn xóa trạm trung chuyển <?php echo e($hub->name); ?>? Đơn hàng liên kết có thể bị ảnh hưởng!')">
                                    <?php echo csrf_field(); ?>
            ...
            <?php if($hubs->hasPages()): ?>
            <div class="px-6 py-4 border-t border-slate-100 bg-slate-50/50">
                <?php echo e($hubs->links()); ?>
...
<div id="createModal" class="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center hidden p-4">
    <div class="bg-white rounded-2xl w-full max-w-lg shadow-2xl border border-slate-100 overflow-hidden">
        ...
        <form action="<?php echo e(route('admin.hubs.store')); ?>" method="POST" class="p-6 space-y-4">
            <?php echo csrf_field(); ?>
            <div>
...
<script>
    ...
    function initSidebarMap() {
        ...
        let hubsData = <?php echo json_encode($allHubs); ?>;
        let markersGroup = [];
...
<?php echo $__env->make('layouts.admin', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\Users\danht\OneDrive\Desktop\toi_uu_giao_van\resources\views/admin/hubs.blade.php ENDPATH**/ ?>