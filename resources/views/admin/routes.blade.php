...
@section('header_title', 'Tối Ưu Tuyến Đường Giao Hàng')
...
<div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
    <div class="bg-white rounded-2xl border border-slate-200/60 p-4 shadow-sm">
        ...
        <span class="text-2xl font-extrabold text-slate-800 mt-1 block">{{ $routes->count() }}</span>
    </div>
    <div class="bg-white rounded-2xl border border-slate-200/60 p-4 shadow-sm">
        <span class="text-[10px] font-bold uppercase text-slate-400 block">Tổng điểm dừng</span>
        <span class="text-2xl font-extrabold text-blue-600 mt-1 block">{{ $routes->sum(fn($r) => $r->routeOrders->count()) }}</span>
    </div>
    <div class="bg-white rounded-2xl border border-slate-200/60 p-4 shadow-sm">
        <span class="text-[10px] font-bold uppercase text-slate-400 block">Quãng đường TB</span>
        <span class="text-2xl font-extrabold text-emerald-600 mt-1 block">{{ $routes->count() ? round($routes->avg('total_distance'), 1) : 0 }} km</span>
    </div>
    <div class="bg-white rounded-2xl border border-slate-200/60 p-4 shadow-sm">
        <span class="text-[10px] font-bold uppercase text-slate-400 block">Thời gian TB</span>
        <span class="text-2xl font-extrabold text-orange-500 mt-1 block">{{ $routes->count() ? round($routes->avg('estimated_time')) : 0 }} phút</span>
    </div>
</div>
...
<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
    ...
    <div class="flex items-center gap-3 flex-wrap">
        <a href="{{ route('admin.dispatch') }}" class="inline-flex items-center px-4 py-2 rounded-xl text-xs font-bold bg-indigo-50 text-indigo-700 border border-indigo-200 hover:bg-indigo-100 transition shadow-sm">
...
<div class="flex border-b border-slate-200 mb-6 gap-2">
    <button onclick="filterRoutes('all')" id="tab-btn-all" class="route-tab-btn py-2 px-4 text-xs font-bold text-blue-600 border-b-2 border-blue-600 transition focus:outline-none">
        Tất cả tuyến (<span>{{ $routes->count() }}</span>)
    </button>
    <button onclick="filterRoutes('unassigned')" id="tab-btn-unassigned" class="route-tab-btn py-2 px-4 text-xs font-bold text-slate-500 hover:text-slate-700 transition focus:outline-none">
        Chưa gán Shipper (<span>{{ $routes->filter(fn($r) => !$r->shipper_id)->count() }}</span>)
    </button>
    <button onclick="filterRoutes('assigned')" id="tab-btn-assigned" class="route-tab-btn py-2 px-4 text-xs font-bold text-slate-500 hover:text-slate-700 transition focus:outline-none">
        Đã gán Shipper (<span>{{ $routes->filter(fn($r) => $r->shipper_id)->count() }}</span>)
    </button>
</div>
...
<div class="space-y-6" id="routes-container">
    ...
    @endphp
    <div class="route-card bg-white rounded-2xl border border-slate-200/60 shadow-sm overflow-hidden" data-assigned="{{ $route->shipper_id ? 'true' : 'false' }}">
        <div class="px-6 py-4 bg-slate-50 border-b border-slate-150 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div class="flex items-center space-x-3">
                <div class="h-10 w-10 rounded-xl bg-blue-50 text-blue-600 flex items-center justify-center text-lg font-bold border border-blue-100">{{ $route->id }}</div>
                <div>
                    <h4 class="text-sm font-bold text-slate-900">Tuyến đường #{{ $route->id }}</h4>
                    <p class="text-xs text-slate-400">Shipper: <span class="font-bold {{ $route->shipper ? 'text-emerald-700' : 'text-rose-500' }}">
                        {{ $route->shipper->user->username ?? '⚠ Chưa gán shipper' }}
                    </span></p>
                </div>
                @if(!$route->shipper)
            ...
            <div class="flex flex-wrap items-center gap-3 text-xs">
                <div class="bg-white border border-slate-200 rounded-lg px-3 py-1.5 text-center">
                    ...
                    <span class="font-extrabold text-slate-800">{{ $route->total_distance }} km</span>
                </div>
                <div class="bg-white border border-slate-200 rounded-lg px-3 py-1.5 text-center">
                    <span class="text-[9px] uppercase font-bold text-slate-400 block">Ước tính</span>
                    <span class="font-extrabold text-slate-800">{{ $route->estimated_time }} phút</span>
                </div>
                <div class="bg-white border border-slate-200 rounded-lg px-3 py-1.5 text-center">
                    <span class="text-[9px] uppercase font-bold text-slate-400 block">Điểm dừng</span>
                    <span class="font-extrabold text-blue-600">{{ $route->routeOrders->count() }}</span>
                </div>
                <button onclick="openRouteMapModal({{ $route->id }}, {{ json_encode($hubData) }}, {{ json_encode($route->routeOrders) }})"
                ...
                <button onclick="openAssignModal({{ $route->id }}, '{{ $route->shipper->user->username ?? 'Chua gan' }}')"
                ...
                <button onclick="openEditRouteModal({{ $route->id }}, {{ $route->total_distance }}, {{ $route->estimated_time }})"
                ...
                <form action="{{ route('admin.routes.destroy', $route->id) }}" method="POST"
                    onsubmit="return confirm('Xóa tuyến #{{ $route->id }}? Tất cả đơn hàng sẽ bị bỏ gán Shipper.')">
                    @csrf @method('DELETE')
        ...
        <div class="p-6">
            ...
            <div class="relative pl-6 space-y-4">
                ...
                @forelse($route->routeOrders as $stop)
                    ...
                    <div class="flex-1 grid grid-cols-1 md:grid-cols-4 gap-3">
                        ...
                        <div class="md:col-span-3">
                            <div class="flex items-center gap-2 mb-1">
                                <span class="font-bold text-slate-800">{{ $stop->order->order_code ?? 'N/A' }}</span>
                                <span class="text-slate-400">•</span>
                                <span class="text-slate-500">{{ $stop->order->customer->user->username ?? 'N/A' }}</span>
                            </div>
                            <div class="space-y-0.5 text-slate-500">
                                <div class="flex items-center"><span class="w-1.5 h-1.5 bg-blue-400 rounded-full mr-1.5"></span> {{ $stop->order->pickup_address ?? 'N/A' }}</div>
                                <div class="flex items-center"><span class="w-1.5 h-1.5 bg-emerald-400 rounded-full mr-1.5"></span> {{ $stop->order->delivery_address ?? 'N/A' }}</div>
                            </div>
                        </div>
                    </div>
                </div>
...
<div id="createRouteModal" class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 hidden">
    <div class="bg-white rounded-2xl w-full max-w-lg shadow-2xl border border-slate-100">
        ...
        <form action="{{ route('admin.routes.store') }}" method="POST" class="p-6 space-y-4">
            @csrf
            <div>
                ...
                <select name="shipper_id" required class="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-xs bg-slate-50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500">
                    ...
                    @foreach(\App\Models\Shipper::with('user')->get() as $s)
                    <option value="{{ $s->id }}">{{ $s->user->username ?? 'N/A' }} ({{ $s->vehicle_type }})</option>
                    @endforeach
                </select>
            </div>
            <div>
                ...
                <select name="order_ids[]" multiple required class="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-xs bg-slate-50 h-36 focus:outline-none focus:ring-2 focus:ring-blue-500">
                    @foreach(\App\Models\Order::whereIn('status', ['pending'])->where('assignment_status', 'unassigned')->get() as $o)
                    <option value="{{ $o->id }}">{{ $o->order_code }} – {{ Str::limit($o->delivery_address, 40) }}</option>
                    @endforeach
                </select>
