...
<div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
    ...
    <div class="bg-white rounded-2xl p-4 border border-slate-200/60 shadow-sm">
        ...
        <span class="text-xl font-extrabold text-amber-600 mt-1 block">{{ $stats['processing'] }}</span>
    ...
    <div class="bg-white rounded-2xl p-4 border border-slate-200/60 shadow-sm">
        ...
        <span class="text-xl font-extrabold text-blue-600 mt-1 block">{{ number_format($stats['total_fee']) }}đ</span>
        <span class="text-[10px] text-slate-400 mt-1 block">Khối lượng: {{ number_format($stats['total_weight'], 1) }}kg</span>
    </div>
</div>
...
<form action="{{ route('admin.orders') }}" method="GET" class="bg-white rounded-2xl border border-slate-200/60 shadow-sm p-5 mb-6 space-y-4">
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div class="md:col-span-2 flex gap-2">
            <div class="w-1/3">
                ...
                <select name="search_type" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2.5 text-xs focus:bg-white focus:outline-none">
                    <option value="all" {{ request('search_type') === 'all' ? 'selected' : '' }}>Tất cả trường</option>
                    <option value="code" {{ request('search_type') === 'code' ? 'selected' : '' }}>Mã đơn hàng</option>
                    <option value="address" {{ request('search_type') === 'address' ? 'selected' : '' }}>Địa chỉ giao nhận</option>
                    <option value="customer" {{ request('search_type') === 'customer' ? 'selected' : '' }}>Tên / SĐT Khách</option>
                </select>
            </div>
            <div class="w-2/3">
                <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Từ khóa</label>
                <div class="relative">
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Mã đơn, địa chỉ, khách..." class="w-full bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2.5 text-xs focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500">
                    @if(request('search'))
                        <a href="{{ route('admin.orders', request()->except('search')) }}" class="absolute right-3 top-3 text-slate-400 hover:text-slate-650">✕</a>
                    @endif
                </div>
            </div>
        </div>
        ...
        <div>
            ...
            <select name="status" onchange="this.form.submit()" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2.5 text-xs focus:bg-white focus:outline-none">
                ...
                <option value="processing" {{ request('status') === 'processing' ? 'selected' : '' }}>Đang giao (Processing)</option>
                <option value="delivered" {{ request('status') === 'delivered' ? 'selected' : '' }}>Đã giao (Delivered)</option>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 border-t border-slate-100 pt-3">
        <div>
            ...
            <select name="shipper_id" onchange="this.form.submit()" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2.5 text-xs focus:bg-white focus:outline-none">
                ...
                @foreach($shippers as $s)
                    <option value="{{ $s->id }}" {{ request('shipper_id') == $s->id ? 'selected' : '' }}>{{ $s->user->username ?? '' }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div>
            ...
            <input type="number" name="min_fee" value="{{ request('min_fee') }}" placeholder="Ví dụ: 10000" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2.5 text-xs focus:bg-white focus:outline-none">
        </div>

        <div>
            <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Cước tối đa (đ)</label>
            <input type="number" name="max_fee" value="{{ request('max_fee') }}" placeholder="Ví dụ: 100000" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2.5 text-xs focus:bg-white focus:outline-none">
        </div>
            ...
            <tbody class="divide-y divide-slate-100 text-sm text-slate-700">
                ...
                <tr class="hover:bg-slate-50/20 transition">
                    ...
                    <td class="px-6 py-4 font-bold text-blue-600">
                        <button type="button" onclick="openTrackingModal({{ json_encode($order) }})" class="hover:underline focus:outline-none text-left">
                            {{ $order->order_code }}
                        </button>
                        <span class="block text-[10px] text-slate-400 font-mono">Hub: {{ $order->hub->name ?? 'N/A' }}</span>
                    </td>
                    <!-- Customer -->
                    <td class="px-6 py-4">
                        <div class="font-semibold text-slate-900">{{ $order->customer->user->username ?? 'Khách lẻ' }}</div>
                        <div class="text-[11px] text-slate-400">{{ $order->customer->user->phone ?? '' }}</div>
                    </td>
                    <!-- Path -->
                    <td class="px-6 py-4">
                        <div class="space-y-1 text-xs">
                            <div class="flex items-center text-slate-650">
                                ...
                                <span class="truncate max-w-[200px]" title="{{ $order->pickup_address }}">{{ $order->pickup_address }}</span>
                            </div>
                            <div class="flex items-center text-slate-650">
                                <span class="w-1.5 h-1.5 bg-emerald-500 rounded-full mr-2"></span>
                                <span class="truncate max-w-[200px]" title="{{ $order->delivery_address }}">{{ $order->delivery_address }}</span>
                            </div>
                        </div>
                    </td>
                    <!-- weight / fee -->
                    <td class="px-6 py-4 text-xs">
                        <div><span class="text-slate-400">Nặng:</span> <span class="font-semibold text-slate-800">{{ $order->total_weight }} kg</span></div>
                        <div class="mt-0.5"><span class="text-slate-400">Phí:</span> <span class="font-semibold text-slate-900">{{ number_format($order->shipping_fee) }}đ</span></div>
                    </td>
                    <!-- Shipper -->
                    <td class="px-6 py-4">
                        @if($order->shipper)
                            <div class="font-semibold text-slate-900">{{ $order->shipper->user->username ?? '' }}</div>
                            <div class="text-[10px] text-slate-400">{{ $order->shipper->license_no ?? '' }}</div>
                        @else
                    ...
                    <td class="px-6 py-4 text-right text-xs font-bold space-x-2.5 flex justify-end items-center">
                        <a href="{{ route('admin.orders.print', $order->id) }}" target="_blank" class="text-slate-500 hover:text-slate-800 hover:underline mr-1" title="In Vận Đơn">In</a>
                        <button onclick="openTrackingModal({{ json_encode($order) }})" class="text-emerald-600 hover:text-emerald-800 hover:underline">Hành trình</button>
                        <button onclick="openEditModal({{ json_encode($order) }})" class="text-blue-600 hover:text-blue-800 hover:underline">Sửa</button>
                        <form action="{{ route('admin.orders.destroy', $order->id) }}" method="POST" class="inline" onsubmit="return confirm('Bạn có chắc chắn muốn xóa đơn hàng {{ $order->order_code }} không?')">
                            @csrf
                            @method('DELETE')
...
<div id="createModal" class="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center hidden p-4">
    <div class="bg-white rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl border border-slate-100">
        ...
        <form action="{{ route('admin.orders.store') }}" method="POST" class="p-6 space-y-4">
            @csrf
                ...
                <div>
                    ...
                    <select name="customer_id" required class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:bg-white focus:outline-none">
                        @foreach($customers as $c)
                            <option value="{{ $c->id }}">{{ $c->user->username ?? '' }} ({{ $c->membership_level ?? 'Normal' }})</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="shipper_id" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:bg-white focus:outline-none">
                        ...
                        @foreach($shippers as $s)
                            <option value="{{ $s->id }}">{{ $s->user->username ?? '' }} ({{ $s->license_no }})</option>
                        @endforeach
                    </select>
                </div>
                <div>
